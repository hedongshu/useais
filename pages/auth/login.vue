...
import useNaiveForm from '~~/composables/naive/useNaiveForm'
definePageMeta({
  layout: 'auth',
  middleware: 'guest',
})
...
async function handleSubmit() {
  ...
  if (error.value) {
    apiErrors.value.wrongCredentials =
      error.value.data?.message === 'wrong-credentials'
    apiErrors.value.accountNotVerified =
      error.value.data?.message === 'account-not-verified'
    apiErrors.value.accountSuspended =
      error.value.data?.message === 'account-suspended'
  }
}
</script>
